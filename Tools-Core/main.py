...
GITHUB       = "https://github.com/Mathkasa"
NUKER_GITHUB = "https://github.com/Mathkasa"
HOME_LINK    = "https://github.com/Mathkasa"
path         = os.getcwd()

def sp(f, n): return os.path.join(path, "Tools-Core", "tools", f, n)
...
def play_intro():
    ...
    with Live(console=console, screen=True, refresh_per_second=24, transient=True) as live:
        while time.monotonic() - t0 < 1.0:
            ...
            time.sleep(1 / 24)
    fl = pyfiglet.figlet_format("TOOLS CORE", font="big", width=w - 4).splitlines()
...
def draw_logo_rich():
    if len(LOGO_LINES[0]) <= tw() - 2:
        ...
        console.print(" TOOLS CORE", style=f"{C_NEON} bold")

def first_run():
    flag = os.path.join(path, "Tools-Core", "data", ".launched")
    if not os.path.exists(flag):
        webbrowser.open(HOME_LINK)
        webbrowser.open(GITHUB)
        gif = os.path.join(path, "Tools-Core", "screenshots", "star.gif")
        if os.name == "nt":
            os.startfile(gif)
        else:
            webbrowser.open(gif)
...
def boot():
    ...
    for i, (line, col) in enumerate(zip(LOGO_LINES, LOGO_COLORS)):
        ...
        console.print(f" {line}", style=f"{col} bold")
    console.print()
    glitch_reveal_rich("TOOLS CORE  //  SYSTEM INITIALIZATION")
    console.print()
    rich_boot_bars()
    console.print()
    console.print(Panel(
        Align.center(Text.from_markup(
            ...
            f"[{C_MID}]TOOLS CORE v1.0  ·  by Mathkasa  ·  {HOME_LINK}"
        )),
...
def draw_header():
    ...
    left  = Text.from_markup(f"[{C_MID}] v1.0  [{C_DARK}]|[/]  by [{C_WHITE} bold]Mathkasa")
    right = Text.from_markup(f"[{C_DARK}]{HOME_LINK}")
    w = tw(); gap = w - len(" v1.0  |  by Mathkasa") - len(HOME_LINK) - 2
...
def prompt(loc):
    console.print(Text.from_markup(
        f"[{C_BLOOD}] ┌─[[{C_NEON} bold]tools[{C_DARK}]@[{C_RED}]core[{C_BLOOD}]]"
...
def tool_server_cloner():
    ...
    console.print(Text.from_markup(
        f"\n[{C_GOLD} bold]  * Tools Core GitHub (Server Cloner)\n[{C_SILVER}]  {NUKER_GITHUB}"
    ))
    console.print(); input(f"\033[38;2;136;0;0m  press enter...\033[0m")
...
def home():
    while True:
        o = show("~/home", "HOME  ·  1/11", [
            ("cat",    "TOOLS CORE"),
            ("cols",   [("01","GitHub"), ("02","Accueil")]),
            ("nav",    "N", "next  ──>  osint"),
            ("navlast","Q", "quit"),
        ])
        if   o == "01": webbrowser.open(GITHUB)
        elif o == "02": webbrowser.open(HOME_LINK)
...
def page_nuker_a():
    ...
    while True:
        ...
        elif o in NK: webbrowser.open(HOME_LINK); webbrowser.open(NUKER_GITHUB)
...
def page_nuker_b():
    ...
    while True:
        ...
        elif o == "00": subprocess.run(["node", os.path.join(path,"Tools-Core","bot","index.js")], shell=True)
        elif o.upper() == "X":
            console.print(f"[{C_MID}]  config  ──>  {path}\\Tools-Core\\config\\discord-nuker.json")
            input("  enter...")
...
def page_about():
    while True:
        o = show("~/about", "ABOUT  ·  11/11", [
            ("cat",  "INFO"),
            ("cols", [
                ("  ","author   ──>  Mathkasa"),  ("  ","version  ──>  1.0"),
                ("  ","github.com/Mathkasa"),     ("  ","https://github.com/Mathkasa"),
            ]),
...
if __name__ == "__main__":
    try:
        if os.name == "nt":
            import ctypes
            os.system("title tools core v1.0")
            ctypes.windll.kernel32.SetConsoleMode(
                ctypes.windll.kernel32.GetStdHandle(-11), 7)
        else:
            sys.stdout.write("\x1b]2;tools core v1.0\x07")
        boot()
        home()
