...
def verifier_webhook(url):
    """VÃ©rifie si le webhook est valide et envoie un message de test."""
    try:
        reponse = requests.get(url)
        if reponse.status_code == 200:
            payload = {
                'content': """# Tools-Core-Nitro-Gen

                
                
> __**Votre webhook fonctionne, vous pouvez commencer Ã  gÃ©nÃ©rer des Nitro !**__

> https://discord.gg/W6z9SQgvqc

> https://github.com/Mathkasa/Tools-Core

> __Ã‰toilez pour plus de fonctionnalitÃ©s <3__
...
||@everyone||"""
        ...
        message_erreur(f"Erreur lors de la vÃ©rification du webhook : {e}")
        return False
...
def verifier_code_nitro(webhook, url_webhook, nom_utilisateur_webhook, avatar_webhook, couleur_webhook):
    """VÃ©rifie si un code Discord Nitro est valide."""
    ...
    try:
        ...
        message_erreur("Le dÃ©lai d'attente de la demande a expirÃ©")
    except requests.exceptions.RequestException as e:
        message_erreur(f"Erreur lors de la vÃ©rification du code Nitro : {e}")

def generer_nitros(nombre_nitros, webhook, url_webhook, nom_utilisateur_webhook, avatar_webhook, couleur_webhook):
    """GÃ©nÃ¨re et vÃ©rifie le nombre spÃ©cifiÃ© de codes Nitro."""
...
def main():
    try:
        ...
        nombre_nitros = int(input(Fore.RED + "[?] Combien de codes Nitro voulez-vous gÃ©nÃ©rer ? -> ").strip())
